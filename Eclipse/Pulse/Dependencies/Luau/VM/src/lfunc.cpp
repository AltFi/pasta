...
Closure* luaF_newLclosure(lua_State* L, int nelems, LuaTable* e, Proto* p)
{
    ...
    c->preload = 0;
    // usage tracking removed -- see Closure struct comment in lobject.h (dropped to match a real 72-byte live-process dump; was never read anywhere, only counted)
    c->l.p = p;
...
Closure* luaF_newCclosure(lua_State* L, int nelems, LuaTable* e)
{
    ...
    c->preload = 0;
    // usage tracking removed -- see Closure struct comment in lobject.h
    c->c.f = NULL;
    c->c.cont = NULL;
    c->c.debugname = NULL;
    return c;
